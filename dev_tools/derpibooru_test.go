...
func TestDerpibooruPrototype(t *testing.T) {
	...
	baseURL := os.Getenv("DERPIBOORU_URL")
	if baseURL == "" {
	...
	apiKey := os.Getenv("DERPIBOORU_API_KEY")

	params := url.Values{}
	// query some safe tags for the prototype test
	params.Add("q", "safe, pony")
