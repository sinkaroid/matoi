...
// KonachanNetPost represents a raw post response from the KonachanNet JSON API.
type KonachanNetPost struct {
	ID         int    `json:"id"`
...
// KonachanNetProvider manages fetching posts from KonachanNet.
type KonachanNetProvider struct {
	Cfg *config.Config
}

// NewKonachanNetProvider creates a new KonachanNet provider instance.
func NewKonachanNetProvider(cfg *config.Config) *KonachanNetProvider {
	return &KonachanNetProvider{Cfg: cfg}
}

// FetchPosts fetches posts from the KonachanNet API based on tags, limit, and page.
func (p *KonachanNetProvider) FetchPosts(ctx context.Context, tags string, limit, page int) ([]models.Post, error) {
	tr := &http.Transport{
...
func (p *KonachanNetProvider) buildURL(tags string, limit, page int) (string, error) {
	u, err := url.Parse("https://konachan.net/post.json")
	if err != nil {
		return "", fmt.Errorf("failed to parse KonachanNet URL: %w", err)
	}

	q := u.Query()
	q.Set("tags", tags)
	q.Set("limit", strconv.Itoa(limit))
	q.Set("page", strconv.Itoa(page)) // KonachanNet uses 1-based page index

	u.RawQuery = q.Encode()
	return u.String(), nil
}

func mapKonachanNetRating(rawRating string) string {
...
func mapKonachanNetPosts(rawPosts []KonachanNetPost) []models.Post {
	...
	for i := range rawPosts {
		...
		posts[i] = models.Post{
			...
			Rating:     mapKonachanNetRating(rp.Rating),
			Score:      rp.Score,
			Source:     rp.Source,
			Tags:       strings.Fields(rp.Tags),
			Link:       fmt.Sprintf("https://konachan.net/post/show/%d", rp.ID),
		}
	}
	return posts
}
