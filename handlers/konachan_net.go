...
// KonachanNetHandler handles HTTP requests for KonachanNet API endpoints.
type KonachanNetHandler struct {
	Provider *providers.KonachanNetProvider
}

// NewKonachanNetHandler creates a new handler instance for KonachanNet.
func NewKonachanNetHandler(p *providers.KonachanNetProvider) *KonachanNetHandler {
	return &KonachanNetHandler{Provider: p}
}

// GetPosts fetches posts from KonachanNet with pagination and caching.
//
...
//	@Security	ApiKeyAuth
//	@Router		/api/konachan_net/posts [get]
//
//nolint:gocyclo,gocognit // Parsing request parameters makes this function complex
func (h *KonachanNetHandler) GetPosts(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	...
	maxLimit, err := strconv.Atoi(h.Provider.Cfg.KonachanNetReturnLimit)
	...
	ctx := c.Context()
	cacheKey := fmt.Sprintf("konachan_net:posts:%s:%d:%d", tags, limit, page)
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, fmt.Sprintf("failed to fetch from KonachanNet: %v", err))
	}
...
//	@Security	ApiKeyAuth
//	@Router		/api/konachan_net/query_completion [get]
func (h *KonachanNetHandler) QueryCompletion(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	if tags == "" {
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, fmt.Sprintf("failed to fetch completion from KonachanNet: %v", err))
	}
...
//	@Router		/api/konachan_net/media [get]
func (h *KonachanNetHandler) ProxyMedia(c fiber.Ctx) error {
	...
	req.Header.Set("Referer", "https://konachan.net/")
...
func (h *KonachanNetHandler) resolveMatoiURLs(baseURL string, posts []models.Post) {
	...
	for i := range posts {
		p := &posts[i]
		if p.FileURL != "" {
			p.MatoiFileURL = fmt.Sprintf("%s/api/konachan_net/media?url=%s", baseURL, url.QueryEscape(p.FileURL))
		}
		if p.PreviewURL != "" {
			p.MatoiPreviewURL = fmt.Sprintf("%s/api/konachan_net/media?url=%s", baseURL, url.QueryEscape(p.PreviewURL))
		}
		if p.SampleURL != "" {
			p.MatoiSampleURL = fmt.Sprintf("%s/api/konachan_net/media?url=%s", baseURL, url.QueryEscape(p.SampleURL))
		}
	}
}
