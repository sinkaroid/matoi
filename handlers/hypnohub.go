...
// HypnohubHandler handles HTTP requests for Hypnohub API endpoints.
type HypnohubHandler struct {
	Provider *providers.HypnohubProvider
}

// NewHypnohubHandler creates a new handler instance for Hypnohub.
func NewHypnohubHandler(p *providers.HypnohubProvider) *HypnohubHandler {
	return &HypnohubHandler{Provider: p}
}

// GetPosts fetches posts from Hypnohub with pagination and caching.
//
...
//	@Security	ApiKeyAuth
//	@Router		/api/hypnohub/posts [get]
//
//nolint:gocyclo,gocognit // Parsing request parameters makes this function complex
func (h *HypnohubHandler) GetPosts(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	...
	ctx := c.Context()
	cacheKey := fmt.Sprintf("hypnohub:posts:%s:%d:%d", tags, limit, page)
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, fmt.Sprintf("failed to fetch from Hypnohub: %v", err))
	}
...
//	@Security	ApiKeyAuth
//	@Router		/api/hypnohub/query_completion [get]
func (h *HypnohubHandler) QueryCompletion(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	if tags == "" {
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, fmt.Sprintf("failed to fetch completion from Hypnohub: %v", err))
	}
...
//	@Router		/api/hypnohub/media [get]
func (h *HypnohubHandler) ProxyMedia(c fiber.Ctx) error {
...
func (h *HypnohubHandler) resolveMatoiURLs(baseURL string, posts []models.Post) {
	...
	for i := range posts {
		p := &posts[i]
		if p.FileURL != "" {
			p.MatoiFileURL = fmt.Sprintf("%s/api/hypnohub/media?url=%s", baseURL, url.QueryEscape(p.FileURL))
		}
		if p.PreviewURL != "" {
			p.MatoiPreviewURL = fmt.Sprintf("%s/api/hypnohub/media?url=%s", baseURL, url.QueryEscape(p.PreviewURL))
		}
		if p.SampleURL != "" {
			p.MatoiSampleURL = fmt.Sprintf("%s/api/hypnohub/media?url=%s", baseURL, url.QueryEscape(p.SampleURL))
		}
	}
}
