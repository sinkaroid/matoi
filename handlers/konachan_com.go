...
// KonachanComHandler handles HTTP requests for KonachanCom API endpoints.
type KonachanComHandler struct {
	Provider *providers.KonachanComProvider
}

// NewKonachanComHandler creates a new handler instance for KonachanCom.
func NewKonachanComHandler(p *providers.KonachanComProvider) *KonachanComHandler {
	return &KonachanComHandler{Provider: p}
}

// GetPosts fetches posts from KonachanCom with pagination and caching.
//
...
//	@Security	ApiKeyAuth
//	@Router		/api/konachan_com/posts [get]
//
//nolint:gocyclo,gocognit // Parsing request parameters makes this function complex
func (h *KonachanComHandler) GetPosts(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	...
	ctx := c.Context()
	cacheKey := fmt.Sprintf("konachan_com:posts:%s:%d:%d", tags, limit, page)
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, fmt.Sprintf("failed to fetch from KonachanCom: %v", err))
	}
...
//	@Security	ApiKeyAuth
//	@Router		/api/konachan_com/query_completion [get]
func (h *KonachanComHandler) QueryCompletion(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	if tags == "" {
	...
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, fmt.Sprintf("failed to fetch completion from KonachanCom: %v", err))
	}
...
//	@Router		/api/konachan_com/media [get]
func (h *KonachanComHandler) ProxyMedia(c fiber.Ctx) error {
	...
	req.Header.Set("Referer", "https://konachan.com/")
...
func (h *KonachanComHandler) resolveMatoiURLs(baseURL string, posts []models.Post) {
	...
	for i := range posts {
		p := &posts[i]
		if p.FileURL != "" {
			p.MatoiFileURL = fmt.Sprintf("%s/api/konachan_com/media?url=%s", baseURL, url.QueryEscape(p.FileURL))
		}
		if p.PreviewURL != "" {
			p.MatoiPreviewURL = fmt.Sprintf("%s/api/konachan_com/media?url=%s", baseURL, url.QueryEscape(p.PreviewURL))
		}
		if p.SampleURL != "" {
			p.MatoiSampleURL = fmt.Sprintf("%s/api/konachan_com/media?url=%s", baseURL, url.QueryEscape(p.SampleURL))
		}
	}
}
