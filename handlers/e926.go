...
// E926Handler handles HTTP requests for E926.
type E926Handler struct {
	Provider *providers.E926Provider
}

// NewE926Handler creates a new instance of E926Handler.
func NewE926Handler(provider *providers.E926Provider) *E926Handler {
	return &E926Handler{
		Provider: provider,
	}
}

// E926Response defines the JSON response schema for posts query.
...
// GetPosts fetches posts from E926.
//
//	@Summary		Fetch posts from E926
//	@Description	Fetches a list of posts from E926 based on provided tags.
//	@Tags			e926
//	@Produce		json
//	@Security		ApiKeyAuth
...
//	@Success		200		{object}	E926Response
//	@Failure		500		{object}	map[string]string	"Internal server error"
//	@Router			/api/e926/posts [get]
//
//nolint:gocyclo,gocognit // Parsing request parameters makes this function complex
func (h *E926Handler) GetPosts(c fiber.Ctx) error {
	tags := c.Query("tags", "")
	...
	cacheKey := fmt.Sprintf("e926:posts:tags=%s:limit=%d:page=%d", urlQueryEncode(tags), limit, page)

	// Fast path: try cache
	var cachedPosts []models.Post
	found, err := cache.Get(ctx, cacheKey, &cachedPosts)
	if err != nil {
		log.Printf("Redis get error for e926 posts: %v", err)
	}
	if found {
		...
		if len(cachedPosts) == 0 {
			return c.Status(http.StatusNotFound).JSON(E926Response{
				Success:  false,
				Provider: "e926",
				Count:    0,
				Posts:    []models.Post{},
	...
	if len(posts) > 0 {
		go func() {
			...
			if err := cache.Set(bgCtx, cacheKey, posts, h.Provider.Config.RedisExpireCache); err != nil {
				log.Printf("Failed to cache e926 posts: %v", err)
			}
		}()
	}
...
func (h *E926Handler) resolveMatoiURLs(c fiber.Ctx, posts []models.Post) {
	...
	for i := range posts {
		p := &posts[i]
		if p.FileURL != "" {
			p.MatoiFileURL = fmt.Sprintf("%s/api/e926/media?url=%s", baseURL, url.QueryEscape(p.FileURL))
		}
		if p.PreviewURL != "" {
			p.MatoiPreviewURL = fmt.Sprintf("%s/api/e926/media?url=%s", baseURL, url.QueryEscape(p.PreviewURL))
		}
		if p.SampleURL != "" {
			p.MatoiSampleURL = fmt.Sprintf("%s/api/e926/media?url=%s", baseURL, url.QueryEscape(p.SampleURL))
		}
	}
}

// ProxyMedia fetches and streams blocked E926 media.
//
//	@Summary	Proxy and stream E926 media
//	@Tags		e926
//	@Param		url	query	string	true	"Encoded E926 media URL"
//	@Success	200	"Streams the media file"
//	@Failure	400	{object}	map[string]string	"Invalid parameters"
//	@Failure	502	{object}	map[string]string	"Failed to fetch media"
//	@Router		/api/e926/media [get]
//
//nolint:gocyclo // Proxy logic is inherently complex
func (h *E926Handler) ProxyMedia(c fiber.Ctx) error {
	...
	if err != nil || (!strings.HasSuffix(parsedURL.Host, ".e926.net") && parsedURL.Host != "e926.net") {
...
//	@Router			/api/e926/query_completion [get]
func (h *E926Handler) QueryCompletion(c fiber.Ctx) error {
	query := c.Query("tags", "")
	if query == "" {
